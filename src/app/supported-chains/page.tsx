...
export default function SupportedChains() {
  ...
  const supportedChainIds =
    Object.keys(supportedChains?.chains || {}).map(CoinIdMapperCoinGeckoToAdamik) || [];
  const displayedChainIds =
    coinList
      ?.map((coin) => CoinIdMapperCoinGeckoToAdamik(coin.id))
      .filter((id) => supportedChainIds.includes(id) || comingSoonIds.includes(id)) || [];
              ...
              {isCoinListLoading || isLoading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-2 xl:grid-cols-4 gap-4">
                  {coinList?.map((coin) => {
                    ...
                    const isSupported = Object.keys(supportedChains?.chains || {})
