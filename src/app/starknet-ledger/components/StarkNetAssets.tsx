...
import { amountToMainUnit, formatAmountUSD, resolveLogo } from "~/utils/helper";
...
interface StarkNetAssetsProps {
  ...
  handleOpenDeployAccount: (isOpen: boolean) => void;
}
...
export const StarkNetAssets = ({
  ...
  handleOpenDeployAccount,
}: StarkNetAssetsProps) => {
  const { address, isConnected } = useLedgerContext(); // Still need this for display/checks
    ...
    <Card className="min-h-[150px]">
      <CardHeader className="flex flex-row items-center justify-between pb-4">
        ...
        <div className="flex flex-row gap-2">
          <Button
            onClick={() => handleOpenDeployAccount(true)}
