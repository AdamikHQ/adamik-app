...
export const clearAccountStateCache = async ({
  ...
  // Only invalidate queries - don't reset or remove them
  // This completely avoids any CancelledError issues
  try {
    // Just invalidate to mark as stale - this is safe and never cancels
    await queryClientGlobal.invalidateQueries({
      queryKey: ["accountState", chainId, address],
      refetchType: "none", // Don't trigger immediate refetch
    });
    
    // That's it! No removal, no reset, no cancel
    // The stale data will be replaced when new data arrives
    // and will be garbage collected based on gcTime settings
  } catch (error) {
    // Silently handle any errors
    console.debug("Cache invalidation error (non-critical):", error);
  }
};
